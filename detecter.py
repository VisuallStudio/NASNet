...
def detect(opt):
    ...
    elif net_name == 'ofaucresnet':
        net = build_net(net_name)(init_channel=8, ks_list=[3, 5, 7], expand_ratio_list=[2, 4, 6, 8],
                                  depth_list=[2, 3, 4], scale_list=4)
    elif net_name == "ucresnet":
        net = build_net(net_name)(init_channel=8, scale=4)
    ...
    if ofa:
        ...
        es = 2
        ds = 2
        net.set_active_subnet(ks, es, ds, ks, es, ds)
    ...
    for i, sample_batched in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            elif opt.net == "ofaucresnet":
                outputs_net1, outputs_net2 = net(input_var)
        ...
        # print('[%d] output shape:' % i, output_net2.size())
        output = scale_disp(output_net2, (output_net2.size()[0], 540, 960))
        disp = output[:, 0, :, :]
        ptime = time.time()

...
def detect(opt):
    ...
    if net_name in ["gwcnet", "ganet", "psmnet"]:
        ...
        net = build_net(net_name)(init_channel=16, ks_list=[3, 5, 7], expand_ratio_list=[2, 4, 6, 8],
                                  depth_list=[2, 3, 4], scale_list=4)
    elif net_name == "ucresnet":
        net = build_net(net_name)(init_channel=16, scale=6)
    ...
    # summary(net,(6,960,576))
    stat(net, torch.randn(3,height,width).cuda())
    input()
    # INIT LOGGERS

...
class DisparityTrainer(object):
    ...
    def _build_net(self):
        ...
        elif self.net_name in ["ofaucresnet"]:
            self.net = build_net(self.net_name)(init_channel=32, ks_list=[3, 5, 7], expand_ratio_list=[2, 4, 6, 8],
                                                depth_list=[2, 3, 4], scale_list=4)

        elif self.net_name in ['fadnet', 'dispnetcss', 'mobilefadnet', 'slightfadnet', 'tinyfadnet', 'microfadnet',
                               'xfadnet']:
            eratio = 16;
            dratio = 16
        ...
        if self.pretrain == '':
            ...
            if os.path.isfile(self.pretrain):
                model_data = torch.load(self.pretrain, map_location=lambda storage, loc: storage)
    ...
    def train_one_epoch(self, epoch):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            ...
            if self.net_name in ["fadnet", "mobilefadnet", 'slightfadnet', 'tinyfadnet', 'microfadnet', 'xfadnet',
                ...
                if self.net_name == 'ucresnet' or self.net_name == 'ofaucresnet':
                    output_net1.reverse()
                    output_net2.reverse()
    ...
    def validate(self):
                ...
                for d in depth_list:
                    for e in expand_list:
                        subnet_settings.append([{
                            ...
                            'es_up': e,
                            'ds_up': d
                        }, 'K_dn%s-E_dn%s-D_dn%s-K_up%s-E_up%s-D_up%s' % (k, e, d, k, e, d)])
            for setting, name in subnet_settings:
                self.net.set_active_subnet(**setting)
        ...
        # switch to evaluate mode
            ...
            for i, sample_batched in enumerate(self.test_loader):
                ...
                elif self.net_name == 'ucresnet' or self.net_name == 'ofaucresnet':
                    with torch.no_grad():
                        outputs_net1, outputs_net2 = self.net(input_var)
                    output_net1 = outputs_net1[-1]
                    output_net2 = outputs_net2[-1]

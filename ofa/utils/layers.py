...
def set_layer_from_config(layer_config):
    ...
    name2layer = {
        ...
        MBConvLayer_test.__name__: MBConvLayer_test,
...
class MBConvLayer_test(MyModule):

    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu6', use_se=False,
                 groups=None, is_transposeConv=False, depth=4):
        super(MBConvLayer_test, self).__init__()
        ...
        if self.expand_ratio == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                ...
                ('conv', nn.ConvTranspose2d(self.in_channels, feature_dim, 1, 1, 0, bias=False)),
        ...
        depth_conv_modules = [
            ...
            if self.is_transposeConv is False else
            ('conv',
             nn.ConvTranspose2d(feature_dim, feature_dim, kernel_size, stride, pad, output_padding=stride - 1,
        ...
        for i in range(2, self.depth):
            block = nn.Sequential(OrderedDict([
                ('conv', nn.Conv2d(feature_dim, feature_dim, 1, 1, 0, bias=False, groups=1))
                if self.is_transposeConv is False else
                ('conv', nn.ConvTranspose2d(feature_dim, feature_dim, 1, 1, 0, bias=False, groups=1)),
                ('bn', nn.BatchNorm2d(feature_dim)),
        ...
        #     self.depth_layers.add_module(
        #         'conv{}'.format(i - 2), nn.Conv2d(feature_dim, feature_dim, 1, 1, 0, bias=False))
        # else:
        #     self.depth_layers.add_module(
        #         'conv{}'.format(i - 2), nn.ConvTranspose2d(feature_dim, feature_dim, 1, 1, 0, bias=False))
        # self.depth_layers.add_module('bn{}'.format(i - 2), nn.BatchNorm2d(feature_dim))
        # self.depth_layers.add_module('act{}'.format(i - 2), build_activation(self.act_func))

        self.point_linear = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(feature_dim, out_channels, 1, 1, 0, bias=False))
            if self.is_transposeConv is False else
            ('conv', nn.ConvTranspose2d(feature_dim, out_channels, 1, 1, 0, bias=False)),
    ...
    @staticmethod
    def build_from_config(config):
        return MBConvLayer_test(**config)
...
class MBConvLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu6', use_se=False,
                 groups=None, is_transposeConv=False):
        ...
        if self.is_transposeConv is False:
            depth_conv_modules = [
                ('conv', nn.Conv2d(feature_dim, feature_dim, kernel_size, stride, pad, groups=groups, bias=False)),
                # ('conv', nn.Conv2d(self.in_channels, feature_dim, kernel_size, stride, pad,
            ...
            depth_conv_modules = [
                ('conv',
                 nn.ConvTranspose2d(feature_dim, feature_dim, kernel_size, stride, pad, output_padding=stride - 1,
                                    groups=groups, bias=False)),
                # ('conv',
                #  nn.ConvTranspose2d(self.in_channels, feature_dim, kernel_size, stride, pad,
                #                     output_padding=stride - 1, groups=self.in_channels, bias=False)),
                ('bn', nn.BatchNorm2d(feature_dim)),
                ('act', build_activation(self.act_func, inplace=True))
            ]
        if self.use_se:

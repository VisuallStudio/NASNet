...
from ofa.utils.my_modules import MyNetwork, MyConv2d, SEModule, MyConvTranspose2d

__all__ = ['DynamicSeparableConv2d', 'DynamicSeparableConvTranspose2d', 'DynamicConv2d', 'DynamicConvTranspose2d',
           'DynamicGroupConv2d',
...
class DynamicSeparableConv2d(nn.Module):
    ...
    # def __init__(self, max_in_channels, max_out_channels, kernel_size_list, stride=1, dilation=1):
    ...
    def forward(self, x, kernel_size=None):
        ...
        # filters = self.get_active_filter(in_channel, out_channel, kernel_size).contiguous()
        ...
        self.re_organize_middle_weights = filters.detach()
...
class DynamicSeparableConvTranspose2d(nn.Module):
    KERNEL_TRANSFORM_MODE = 1  # None or 1

    # def __init__(self, max_in_channels, max_out_channels, kernel_size_list, stride=1, dilation=1):
    def __init__(self, max_in_channels, kernel_size_list, stride=1, dilation=1):
        super(DynamicSeparableConvTranspose2d, self).__init__()

        self.max_in_channels = max_in_channels
        # even_kernel = [i - 1 for i in kernel_size_list]
        # self.kernel_size_list = even_kernel
        self.kernel_size_list = kernel_size_list
        self.stride = stride
        self.dilation = dilation
        self._ks_set = list(set(self.kernel_size_list))
    ...
    def get_active_filter(self, in_channel, kernel_size):
        ...
        max_kernel_size = max(self.kernel_size_list)
        start, end = sub_filter_start_end(max_kernel_size, kernel_size)
        filters = self.conv.weight[:in_channel, :out_channel, start:end, start:end]

        if self.KERNEL_TRANSFORM_MODE is not None and kernel_size < max_kernel_size:
            start_filter = self.conv.weight[:in_channel, :out_channel, :, :]  # start with max kernel
            for i in range(len(self._ks_set) - 1, 0, -1):
                src_ks = self._ks_set[i]
                if src_ks <= kernel_size:
                    break
    ...
    def forward(self, x, kernel_size=None):
        ...
        # output_padding = 0
        filters = self.conv.weight_standardization(filters) if isinstance(self.conv, MyConvTranspose2d) else filters
        filters = filters.to(x.device)
        y = F.conv_transpose2d(x, filters, None, self.stride, padding, output_padding, in_channel, self.dilation)
        return y


class DynamicConv2d(nn.Module):

    def __init__(self, max_in_channels, max_out_channels, kernel_size=1, stride=1, dilation=1, groups=1):
    ...
    def get_active_filter(self, in_channel, out_channel):
    ...
    def forward(self, x, out_channel=None):
        ...
        filters = self.conv.weight_standardization(filters) if isinstance(self.conv, MyConv2d) else filters
        filters = filters.to(x.device)
        y = F.conv2d(x, filters, None, self.stride, padding, self.dilation, self.group
                     )
        return y
...
class DynamicConvTranspose2d(nn.Module):
    ...
    def __init__(self, max_in_channels, max_out_channels, kernel_size=1, stride=1, dilation=1, groups=1, bias=False):
        ...
        self.bias = bias
        self.conv = nn.ConvTranspose2d(
            self.max_in_channels, self.max_out_channels, self.kernel_size, stride=self.stride, groups=self.group,
            bias=bias,
        )
    ...
    def get_active_filter(self, in_channel, out_channel):
        return self.conv.weight[:in_channel, :out_channel, :, :]
    ...
    def forward(self, x, out_channel=None):
        ...
        y = F.conv_transpose2d(x, filters, None, self.stride, padding,
                               dilation=self.dilation, groups=self.group)

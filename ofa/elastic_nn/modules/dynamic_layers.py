...
from ofa.utils.layers import MBConvLayer, ConvLayer, IdentityLayer, set_layer_from_config, MBConvLayer_test
...
from .dynamic_op import DynamicSeparableConv2d, DynamicSeparableConvTranspose2d, DynamicConv2d, DynamicConvTranspose2d, \
    DynamicBatchNorm2d, DynamicSE, \
    DynamicGroupNorm
...
class DynamicMBConvLayerTest(MyModule):

    def __init__(self, in_channel_list, out_channel_list,
                 kernel_size_list=3, expand_ratio_list=6, stride=1, act_func='relu6', use_se=False,
                 is_transposeConv=False, depth_list=4):
        super(DynamicMBConvLayerTest, self).__init__()
        ...
        self.inverted_bottleneck = nn.Sequential(OrderedDict([
            ...
            if self.is_transposeConv is False else
            ('conv', DynamicConvTranspose2d(max(self.in_channel_list), max_middle_channel)),
        ...
        self.depth_conv = nn.Sequential(OrderedDict([
            ...
            if self.is_transposeConv is False else
            ('conv', DynamicSeparableConvTranspose2d(max_middle_channel, self.kernel_size_list, self.stride)),
            # ('conv',
            #  DynamicSeparableConv2d(max(self.in_channel_list), max_middle_channel, self.kernel_size_list,
            #                         self.stride)),
            ('bn', DynamicBatchNorm2d(max_middle_channel)),
            ('act', build_activation(self.act_func))
        ]))
        ...
        for i in range(2):
            block = nn.Sequential(OrderedDict([
                ('conv', DynamicConv2d(max_middle_channel, max_middle_channel, groups=1))
                if self.is_transposeConv is False else
                ('conv', DynamicConvTranspose2d(max_middle_channel, max_middle_channel, groups=1)),
                ('bn', DynamicBatchNorm2d(max_middle_channel)),
                ('act', build_activation(self.act_func)),
            ]))
            self.depth_layers.append(block)
        self.depth_layers = nn.ModuleList(self.depth_layers)

        self.point_linear = nn.Sequential(OrderedDict([
            ('conv', DynamicConv2d(max_middle_channel, max(self.out_channel_list)))
            if self.is_transposeConv is False else
            ('conv', DynamicConvTranspose2d(max_middle_channel, max(self.out_channel_list))),
        ...
        self.active_depth = max(self.depth_list)
    ...
    def forward(self, x):
        ...
        self.depth_conv.conv.active_kernel_size = self.active_kernel_size\
            # if self.is_transposeConv is False \
            # else self.active_kernel_size - 1
        self.point_linear.conv.active_out_channel = self.active_out_channel

        x = self.inverted_bottleneck(x)
        x = self.depth_conv(x)
        for i in range(2, self.active_depth):
            x = self.depth_layers[i - 2](x)
    ...
    def config(self):
        return {
            'name': DynamicMBConvLayerTest.__name__,
    ...
    @staticmethod
    def build_from_config(config):
        return DynamicMBConvLayerTest(**config)
    ...
    def get_active_subnet_test(self, in_channel, preserve_weight=True):
        # build the new layer
        ...
        if sub_layer.inverted_bottleneck is not None:
            sub_layer.inverted_bottleneck.conv.weight.data.copy_(
                self.inverted_bottleneck.conv.get_active_filter(in_channel, middle_channel).data)
        ...
        for i in range(2, sub_layer.depth):
            sub_layer.depth_layers[i - 2].conv.weight.data.copy_(
                self.depth_layers[i - 2].conv.get_active_filter(middle_channel, middle_channel).data
            )
            copy_bn(sub_layer.depth_layers[i - 2].bn, self.depth_layers[i - 2].bn.bn)
            # if i == 2:
            #     sub_layer.depth_layers.conv0.weight.data.copy_(
            #         self.depth_layers[i - 2].conv.get_active_filter(middle_channel, middle_channel).data
            #     )
            #     copy_bn(sub_layer.depth_layers.bn0, self.depth_layers[i - 2].bn.bn)
            # elif i == 3:
            #     sub_layer.depth_layers.conv1.weight.data.copy_(
            #         self.depth_layers[i - 2].conv.get_active_filter(middle_channel, middle_channel).data
            #     )
            #     copy_bn(sub_layer.depth_layers.bn1, self.depth_layers[i - 2].bn.bn)

        sub_layer.point_linear.conv.weight.data.copy_(
            self.point_linear.conv.get_active_filter(middle_channel, self.active_out_channel).data
        )
    ...
    def get_active_subnet_config(self, in_channel):
        return {
            'name': MBConvLayer_test.__name__,
            ...
            'act_func': self.act_func,
            'use_se': self.use_se,
            'is_transposeConv': self.is_transposeConv,
            'depth': self.active_depth,
        }

    def re_organize_middle_weights(self, expand_ratio_stage=0, is_transpose: bool = False):
        # sort importacne by inchannel [out_c, in_c/g ,k, k]
        importance = torch.sum(torch.abs(self.point_linear.conv.conv.weight.data),
                               dim=(0 if is_transpose is False else 1, 2, 3))
        ...
        self.point_linear.conv.conv.weight.data = torch.index_select(
            ...
            1 if is_transpose is False else 0, sorted_idx)
        ...
        adjust_bn_according_to_idx(self.depth_layers[0].bn.bn, sorted_idx)
        self.depth_layers[0].conv.conv.weight.data = torch.index_select(
            self.depth_layers[0].conv.conv.weight.data, 1, sorted_idx)

        adjust_bn_according_to_idx(self.depth_layers[1].bn.bn, sorted_idx)
        self.depth_layers[1].conv.conv.weight.data = torch.index_select(
            self.depth_layers[1].conv.conv.weight.data, 0, sorted_idx)

        if self.inverted_bottleneck is not None:
            adjust_bn_according_to_idx(self.inverted_bottleneck.bn.bn, sorted_idx)
            self.inverted_bottleneck.conv.conv.weight.data = torch.index_select(
                self.inverted_bottleneck.conv.conv.weight.data,
                0 if is_transpose is False else 1, sorted_idx)
            return None
        else:
            return sorted_idx
...
class DynamicMBConvLayer(MyModule):
    ...
    def __init__(self, in_channel_list, out_channel_list,
        ...
        if self.is_transposeConv is False:
            ...
            self.depth_conv = nn.Sequential(OrderedDict([
                ('conv', DynamicSeparableConvTranspose2d(max_middle_channel, self.kernel_size_list, self.stride)),
                # ('conv',
                #  DynamicSeparableConvTranspose2d(max(self.in_channel_list), max_middle_channel, self.kernel_size_list,
                #                                  self.stride)),
                ('bn', DynamicBatchNorm2d(max_middle_channel)),
                ('act', build_activation(self.act_func))
            ]))
    ...
    def re_organize_middle_weights(self, expand_ratio_stage=0, is_transpose: bool = False):
        # sort importacne by inchannel [out_c, in_c/g ,k, k]
        # print("conv weight {}".format(self.point_linear.conv.conv.weight.shape))
        importance = torch.sum(torch.abs(self.point_linear.conv.conv.weight.data),
                               dim=(0 if is_transpose is False else 1, 2, 3))
        # dim=(0, 2, 3)):
        # if is_transpose is True:
        # print(self.point_linear.conv.conv.weight.shape)  #[2048, 128, 1, 1]
        # print('import {}'.format(importance.shape))   #2048
        # input()
        # print("importance weight {}".format(importance.shape))
        if isinstance(self.depth_conv.bn, DynamicGroupNorm):
            channel_per_group = self.depth_conv.bn.channel_per_group
            importance_chunks = torch.split(importance, channel_per_group)
            for chunk in importance_chunks:
        ...
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        # if is_transpose is True:
        #     print(sorted_importance.shape)
        #     print(sorted_idx)
        #     input()
        # print("sorted_importance weight {}".format(sorted_importance.shape))
        # print("sorted_idx {}".format(sorted_idx))
        # print(self.point_linear.conv.conv.weight.shape)
        # if is_transpose is True:
        #     print(self.point_linear.conv.conv.weight.shape)
        self.point_linear.conv.conv.weight.data = torch.index_select(
            self.point_linear.conv.conv.weight.data,
            1 if is_transpose is False else 0, sorted_idx)  # [2048, 128, 1, 1]
        # print(self.point_linear.conv.conv.weight.shape)
        # input()
        # 1 , sorted_idx)
        # print(self.point_linear.conv.conv.weight.shape)
        # print(self.point_linear.conv.conv.weight.data)
        # input()
        # if is_transpose is True:
        #     print(self.depth_conv.conv.conv.weight.shape)  #[2048, 1, 7, 7]
        adjust_bn_according_to_idx(self.depth_conv.bn.bn, sorted_idx)
        self.depth_conv.conv.conv.weight.data = torch.index_select(
            self.depth_conv.conv.conv.weight.data,
            # 0 if is_transpose is False else 1,
            0,
            sorted_idx
        )
